...
# Check Nginx serving frontend
BASE_HOST_URL=$(echo "$SERVER_URL" | sed 's|/api$||')
frontend_status=$(curl -s -w "%{http_code}" -o /dev/null "$BASE_HOST_URL/")
...
cors_res=$(curl -s -i -X OPTIONS -H "Origin: $BASE_HOST_URL" -H "Access-Control-Request-Method: GET" -H "Access-Control-Request-Headers: Authorization" "$SERVER_URL/notes")
if ! echo "$cors_res" | grep -iq "Access-Control-Allow-Origin: \*"; then
    echo "Assertion FAILED: Missing Access-Control-Allow-Origin: *"
    echo "$cors_res"
    exit 1
fi
